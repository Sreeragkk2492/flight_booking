...
class FlightController extends GetxController with GetTickerProviderStateMixin {
  ...
  // These should be the primary source of truth for UI display
  var selectedDeparture = ''.obs;
  var selectedArrival = ''.obs;
  var selectedDepartureAirport = Rxn<Airport>();
  var selectedArrivalAirport = Rxn<Airport>();
  ...
  var passengers = 1.obs;
  var selectedFlight = Rxn<Flight>();
  var isSwapping = false.obs; // Add this to track swap animation state
  ...
  late Animation<double> priceLineAnimation;
  late Animation<double> rotationAnimation;
  late Animation<double> swapRotationAnimation;
  ...
  void _initializeAnimations() {
    ...
    swapRotationAnimation = Tween<double>(
      begin: 0.0,
      end: 1.0,
    ).animate(CurvedAnimation(
      parent: swapAnimationController,
  ...
  // Fix the airport selection methods to properly update observables
  void selectDepartureAirport(Airport airport) {
    selectedDepartureAirport.value = airport;
    selectedDeparture.value = '${airport.city} (${airport.code})';
    ...
    print('Selected departure: ${selectedDeparture.value}'); // Debug
  }
  ...
  void selectArrivalAirport(Airport airport) {
    ...
    print('Selected arrival: ${selectedArrival.value}'); // Debug
  }

  // Fix the swap functionality with proper state management
  void swapAirports() async {
    // Prevent multiple swaps while animation is running
    if (isSwapping.value) return;
    
    isSwapping.value = true;
    
    // Start swap animation
    swapAnimationController.forward();
    
    // Add haptic feedback
    HapticFeedback.mediumImpact();
    
    // Wait for half the animation to complete before swapping
    await Future.delayed(Duration(milliseconds: 300));
    
    // Perform the swap
    final tempAirport = selectedDepartureAirport.value;
    final tempString = selectedDeparture.value;
    
    // Swap airports
    selectedDepartureAirport.value = selectedArrivalAirport.value;
    selectedArrivalAirport.value = tempAirport;
    
    // Swap display strings
    selectedDeparture.value = selectedArrival.value;
    selectedArrival.value = tempString;
    
    // Force UI update
    update();
    
    // Debug prints
    print('After swap - Departure: ${selectedDeparture.value}');
    print('After swap - Arrival: ${selectedArrival.value}');
    
    // Wait for animation to complete
    await Future.delayed(Duration(milliseconds: 300));
    
    // Reset animation
    swapAnimationController.reset();
  ...
  // Helper method to check if both airports are selected
  bool get canSwap => selectedDepartureAirport.value != null && selectedArrivalAirport.value != null;

  // Helper method to get formatted display text
  String get departureDisplayText {
    if (selectedDepartureAirport.value != null) {
      return '${selectedDepartureAirport.value!.city} (${selectedDepartureAirport.value!.code})';
    }
    return selectedDeparture.value.isEmpty ? 'From' : selectedDeparture.value;
  }

  String get arrivalDisplayText {
    if (selectedArrivalAirport.value != null) {
      return '${selectedArrivalAirport.value!.city} (${selectedArrivalAirport.value!.code})';
    }
    return selectedArrival.value.isEmpty ? 'To' : selectedArrival.value;
  }
  ...
  void loadFlightDeals() {
    flightDeals.value = [
      FlightDeal(
        id: '1',
        destination: 'Paris',
        imageUrl: 'ttps://images.unsplash.com/photo-1540959733332-eab4deabeeaf?w=500',
        originalPrice: 899.99,
        dealPrice: 599.99,
        validity: '7 days left',
        country: 'France',
  ...
  void onClose() {
    ...
    priceGraphController.dispose();
    loadingController.dispose();
    swapAnimationController.dispose();
    super.onClose();
  }
}